  ...
  try {
    ...
    if (!id) {
      const rows = await db
        .select()
        .from(documents)
        .where(eq(documents.ownerUserId, owner.id))
    ...
    const [metadata] = await db
      .select()
      .from(documents)
      .where(and(eq(documents.id, id), eq(documents.ownerUserId, owner.id)))
      .limit(1);
    if (!metadata)
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!areas.has(healthArea))
      return Response.json(
        { error: "Choose a valid health area." },
        { status: 400 },
      );
    ...
    if (!contentType)
      return Response.json(
        { error: "Only genuine PDF, JPEG, PNG, and WebP files are accepted." },
        { status: 400 },
      );
    const id = crypto.randomUUID();
    storedKey = `${await ownerPrefix(owner.id)}/${id}`;
    ...
    if (storedKey) {
      try {
        await (await getBucket()).delete(storedKey);
      } catch {}
    }
    return Response.json(
      {
